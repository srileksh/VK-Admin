...
//       // ✅ DEFINE duration BEFORE using it      
//        const duration = await getVideoDuration(lesson.videoFile);
//       const response = await uploadLessonVideo(lesson.videoFile);
//       setLessons((prev) =>
//       prev.map((l) =>
//         l.id === lesson.id
//       ? {                 ...l,
...
//       videoUploaded: false,
//       backendId: null, 
//       //Reset backend ID strictly speaking? Or allow replace?//               
//     // Ideally we'd delete from backend, but for now just reset UI.
//     }
//     : l,
//   ),
...
//     });
//   } else {
//     savedLesson = await updateLesson(lesson.backendId, {
//       title: lesson.lessonTitle,
//       description: lesson.description,
//       thumbnail: currentThumbnailUrl,
//     });
...
//                       <input
//                       disabled={lesson.isSaved}
//                        className="border border-gray-400 px-4 py-2 rounded-lg w-[250px] outline-gray-400"                        placeholder="Title"//                         value={lesson.lessonTitle}//                         onChange={(e) =>//                           handleUpdateLesson(//                             lesson.id,//                             "lessonTitle",//                             e.target.value,//                           )//                         }//                       />
//                       <input
//                       disabled={lesson.isSaved}
//                       className="border border-gray-400 px-4 py-2 rounded-lg flex-1 outline-gray-400"//                         placeholder="Description"//                         value={lesson.description}//                         onChange={(e) =>//                           handleUpdateLesson(//                             lesson.id,//                             "description",//                             e.target.value,//                           )//                         }//                       />//                     </div>//                     {/* Video Section */}//                     <div className="flex gap-6">//                       <div className="flex gap-3">//                         <div className="w-24 h-20 border border-gray-400 rounded flex items-center justify-center text-xs">//                           <button//                             onClick={() => fileRefs.current[lesson.id].click()}//                             disabled={lesson.isSaved}//                           >//                             {lesson.videoFile//                               ? "Video Selected"//                               : "Select Video"}//                           </button>//                         </div>
//                         <div>//                           <p className="text-sm truncate max-w-[150px]">//                             {lesson.videoName || "Lesson-video.mp4"}//                           </p>
//                           <button//                             onClick={() => handleUploadVideo(lesson)}//                             className="text-sm mt-2 gap-[2px] text-[12px] border px-2.5 py-0.5 text-[#37af47] flex justify-center items-center gap-[2px] rounded-[14px] disabled:opacity-60"//                             disabled={//                               uploadingLessonId === lesson.id ||//                               lesson.isSaved ||//                               !lesson.videoFile ||//                               lesson.videoUploaded//                             }//                           >//                             <MdOutlineFileUpload />
//                             {uploadingLessonId === lesson.id//                               ? "Uploading..."//                               : "Upload"}//                           </button>
//                           <input//                             ref={(el) => (fileRefs.current[lesson.id] = el)}//                             type="file"//                             hidden//                             accept="video/*"//                             onChange={(e) =>//                               handleVideoSelect(lesson, e.target.files[0])//                             }//                           />//                         </div>//                       </div>
//                       {/* Upload Status Bar */}//                       <div className="flex-1 text-sm">//                         <p className="mb-1">Upload status</p>
//                         <div className="relative w-full h-6">//                           <div className="absolute w-full h-[2px] bg-gray-300 top-1/2" />//                           <div//                             className="absolute h-[2px] bg-green-400 top-1/2"//                             style={{//                               width://                                 uploadingLessonId === lesson.id//                                   ? ${progress}%//                                   : lesson.videoUploaded//                                     ? "100%"//                                     : "0%",//                             }}//                           />//                         </div>
//                         <div className="flex justify-between mt-2">//                           <button//                             onClick={() => fileRefs.current[lesson.id].click()}//                             disabled={!lesson.videoUploaded || lesson.isSaved}//                             className="flex items-center gap-1 disabled:opacity-40"//                           >//                             <TbZoomReplace /> Replace//                           </button>
//                           <button//                             onClick={() => handleRemoveVideo(lesson)}//                             disabled={!lesson.videoUploaded || lesson.isSaved}//                             className="flex items-center gap-1 disabled:opacity-40"//                           >//                             <HiMiniMinus /> Remove//                           </button>//                         </div>//                       </div>//                     </div>//                   </div>
//                   {/* THUMBNAIL */}//                   <div className="flex flex-col items-center">//                     <div//                       onClick={() =>//                         !lesson.isSaved &&//                         thumbnailRefs.current[lesson.id].click()//                       }//                       className="w-40 h-28 bg-gray-300 rounded flex items-center justify-center"//                     >//                       {lesson.thumbnailUrl ? (//                         <img//                           src={lesson.thumbnailUrl}//                           className="w-full h-full object-cover"//                           alt="thumb"//                         />//                       ) : (//                         <div className="text-center text-gray-600">//                           <GrGallery size={22} className="mx-auto mb-1" />//                           <p className="text-xs">No file selected</p>//                         </div>//                       )}//                     </div>//                     <button//                       onClick={() => handleUploadThumbnail(lesson)}//                       disabled={//                         !lesson.thumbnailFile ||//                         lesson.isSaved ||//                         uploadingThumbnailId === lesson.id ||//                         (lesson.thumbnailUrl && !lesson.backendId)//                       }//                       className="text-center mt-2 text-[12px] border px-2.5 py-0.5 text-[#37af47]  rounded-[14px] disabled:opacity-60 flex justify-center items-center gap-[2px]"//                     >//                       <MdOutlineFileUpload />
//                       {uploadingThumbnailId === lesson.id//                         ? "Uploading..."//                         : "Upload"}//                     </button>
//                     <input//                       ref={(el) => (thumbnailRefs.current[lesson.id] = el)}//                       type="file"//                       hidden//                       accept="image/*"//                       onChange={(e) =>//                         handleThumbnailSelect(lesson, e.target.files[0])//                       }//                     />//                   </div>//                 </div>
//                 {/* ACTION BUTTONS */}
//                 <div className="flex justify-end gap-4 mt-6">//                   <button//                     onClick={() => handleEdit(lesson)}//                     disabled={!lesson.isSaved}//                     className="flex items-center px-5 py-1.5 border shadow-[#37af47] shadow-2xl border-[#37af47] text-[#37af47] rounded-[12px] text-sm disabled:opacity-50"//                   >//                     Edit//                   </button>
//                   <button//                     onClick={() => handleSave(lesson)}//                     disabled={//                       lesson.saving ||//                       lesson.isSaved ||//                       !isLessonComplete(lesson)//                     }//                     className="px-4 border py-0.5 bg-[#37af47] text-white rounded-[12px] disabled:opacity-50 flex justify-center items-center gap-[2px]"//                   >//                     <LiaSave className="text-[22px]" />//                     Save//                   </button>//                 </div>//               </div>//             ))}
//             <div className="flex justify-center mt-6">//               <button//                 onClick={handleAddLesson}//                 className="text-[#CCCBCB] text-[18px] font-semibold border-b-2"//               >//                 + Add new video//               </button>//             </div>//           </>//         )}//       </div>//     </>//   );// }
